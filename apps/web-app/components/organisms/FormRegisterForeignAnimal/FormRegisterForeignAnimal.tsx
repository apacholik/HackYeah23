...
type FormRegisterForeignAnimalProps = {
  encounterTypes: Array<EncounterTypeResponse>;
};

export const FormRegisterForeignAnimal = ({ encounterTypes }: FormRegisterForeignAnimalProps) => {
  ...
  const { mutate, isError, isSuccess, isLoading, data } = useMutation({
    ...
    mutationFn: async () => {
      const encounterRegisterResponse = await apiClient.put("Encounter", formState);

      if (photoFile) {
        const fd = new FormData();
        fd.append("ImageFile", photoFile);

        await apiClient.post(`Encounter/${encounterRegisterResponse.data}/Image`, fd);
      }
